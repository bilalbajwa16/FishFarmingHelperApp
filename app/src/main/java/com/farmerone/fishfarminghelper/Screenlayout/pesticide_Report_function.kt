...
import com.farmerone.fishfarminghelper.pesticide
...
@Composable
fun reportpesticide(navController: NavHostController, nofishes: String) {
    val context = (LocalContext.current as? Activity)
    val userObject = Gson().fromJson(nofishes,fishpesticide::class.java)
    Column(
        Modifier

            .verticalScroll(rememberScrollState()))
    {
        toolbar(title_value = "Pesticide " +
                "Quantity", contextval = context)
        image(R.drawable.pesticides)

        pesticidereport( navController,  userObject)
    //    if(flag)
...
@Composable
fun pesticidereport(navController: NavHostController, fishparameter:fishpesticide) {
    Column(Modifier.fillMaxSize()) {


        headings(title = "Total Pesticide Required")

        textes(title =  pesticide(fishparameter) )
